...
public class Fp04CustomClass {
    public static void main(String[] args) {
        List<Course> courseList = List.of(
                new Course("Java Basics", "Programming", 4, 100),
                new Course("Data Structures and Algorithms", "Computer Science", 4, 200),
                new Course("Machine Learning Fundamentals", "Artificial Intelligence", 5, 300),
                new Course("Web Development Bootcamp", "Web Development", 4, 150),
                new Course("Introduction to Python", "Programming", 4, 120),
                new Course("Database Management Essentials", "Database Management", 4, 80),
                new Course("JavaScript for Beginners", "Web Development", 3, 250),
                new Course("Artificial Intelligence Ethics", "Ethics", 4, 90),
                new Course("Advanced Java Concepts", "Programming", 5, 80),
                new Course("Software Testing and QA", "Quality Assurance", 4, 70)
        );
        ...
        // get top 2 element after sorting by number of students
        ...
        //sort the list and skip first 2 elements and get the remaining result
        System.out.println(courseList.stream().sorted(comparingByNumberOfStudentsIncreasing).skip(2).collect(Collectors.toList()));


        //takeWhile will keep elements from the stream as long as the predicate is true. Once the predicate becomes false, takeWhile stops and returns the elements it has collected so far
        System.out.println(courseList.stream().takeWhile(course -> course.getNumberOfStudents() <= 100).collect(Collectors.toList()));


      //dropWhile operates on a Stream and removes elements from it as long as a given predicate is true. Once the predicate becomes false for an element, dropWhile stops removing elements and returns the rest of the Stream.
        System.out.println(courseList.stream().dropWhile(course -> course.getNumberOfStudents() <= 200).collect(Collectors.toList()));

        //returns the last item from a list based on the comparator
        System.out.println(courseList.stream().max(comparingByNumberOfStudentsIncreasing).get());

        //returns min fist from list based on comparator
        ...
        System.out.println(courseList.stream().collect(Collectors.groupingBy(Course::getCategory
        ,Collectors.mapping(Course::getName,Collectors.toList()))));//collectors.mapping takes two input ,first the parameter or attribute with which u want to map and second is the collection in which you want to store those mapped value
